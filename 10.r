...
# a. Consider students marks scored in Physics,Chemistry,Maths,FOC,CAD. Draw the pie chart for the same.
# b. Consider different party candidates’ votes received from public in election. Show the votes received in percentage in pie chart.
# c. Consider the students’ performance in different subjects. (Like student roll.no, their marks, gender). 
#   Show the pie chart for girls and boys students’ performance.
...
for(row in 1:nrow(df)) {
  x <- df[row, 2:6]
  pie(as.numeric(x), labels = x, main = paste('Student',df[row,'name'],'marks distribution'), col = rainbow(length(x)))
